...
5. [2026-09-22 · 流光改为全程循环](2026-09-22-flow-loop.md)：23 秒时长、290% 强度，并取消流光的阶段门控；替代时间线第 45 条。
6. [Hero 参数历史](hero-parameters.md)：每轮视觉调整及当前确认值。
7. [Git 提交快照](git-history.md)：截至上一实现提交 `406e803` 的完整可达提交记录；此后的变更以 Git 实时日志为准。
8. 按任务查阅 [项目规则](../../PROJECT_RULES.md)、[Hero 技术说明](../home-hero-animation.md)、[双语与 SEO](../bilingual-seo.md)。

## 本次交接

- 当前用户确认：手机 X 390px、桌面 X 650px；白场/形成/旋转前/旋转/旋转后/退场为 **0 / 10 / 2 / 4 / 2 / 5 秒**，合计 **23 秒**。
- 主体动画已进入参数确认阶段；后续只在明确反馈下继续微调运动细节。
- 流光最新决定：**不分阶段，全程不停循环，只保留速度可调**，强度 290%。它替代了早先“只覆盖完整X三阶段”的方案，详见[流光循环记录](2026-09-22-flow-loop.md)。
