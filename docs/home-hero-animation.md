...
The approved 2026-09-22 sequence repeats every 20 seconds. Earlier values and their rationale are in [the parameter history](project-log/hero-parameters.md):

| Stage | Default duration | Timeline |
| --- | --- | --- |
| Full-frame white | 1 s | 0–1 s |
| Existing white-to-X formation | 6 s | 1–7 s |
| Complete X before rotation | 1 s | 7–8 s |
| Centred 180° rotation | 4 s | 8–12 s |
| Complete X after rotation | 2 s | 12–14 s |
| Formation replayed backwards | 6 s | 14–20 s |

Formation, rotation and exit share one symmetric S curve, `E(t, k) = t^k / (t^k + (1-t)^k)`, whose peak slope is exactly `k`. At `k = 1` it is linear; higher values rest longer at both ends and accelerate harder through the middle. The earlier quintic curve had a peak slope of 1.875, so the approved default of 3 is a deliberate step up in acceleration; the toolbox exposes 1–8. Crucially `E(1-u) ≡ 1-E(u)` holds at every `k`, so changing the easing never breaks the exit's mirror of the formation. Rotation therefore has a slow departure, mid acceleration and a soft arrival at exactly 180°. Coordinates rotate before the reference aspect ratio is applied, so the silhouette never stretches; the background rotates while text remains upright. Each cycle keeps turning rather than snapping back to 0°: `angle = π × (turns + ease)` with `turns = floor(elapsed / total) % 2`. The silhouette is 180°-symmetric so this is invisible in itself, but the flow light is evaluated in rotated coordinates, and resetting the angle would mirror the light across the loop seam; the modulo keeps the value bounded and returns it to 0° every second cycle.
...
Each of the six stage durations can be edited independently in seconds (0–30 for holds, 1–30 for transitions). Timing changes immediately restart the sequence and persist under `xions-preview-timing-v2`; the easing power persists under `xions-preview-ease-v1`. The timeline can be scrubbed to pause at a specific time, with the current stage and rotation angle displayed; playback and replay controls are available alongside it. These saved preferences affect this browser's local toolbox only. The runtime observes both the hero and poster dimensions so height changes update even when playback is paused.

The toolbox can also save the whole parameter set as a named version and load any of them back, so hand-tuned values survive a browser reset and can be read directly from disk by whoever picks the work up next. Versions are written to `local-materials/hero-presets/`, which is git-ignored and never deployed; they remain review values, not site defaults. This is served by `scripts/design-presets-dev.mjs`, a Vite plugin with `apply: 'serve'`, so it exists only on the dev server. Preset names are sanitised to letters, digits, spaces, underscores and hyphens, and the resolved write path is re-checked against the presets directory, so a name cannot escape it.

The toolbox route and launcher are absent from built deployments, and its script is behind an `import.meta.env.DEV` guard so the bundle is dropped from production too. Local updates communicate through development-only custom events. The visitor-facing pause/resume button shows only its icon, with translated screen-reader text retained.
