...
- 六段 **0 / 10 / 2 / 4 / 2 / 5秒 = 23秒**。宽柔光+弧边细高光**不分阶段、全程不停循环**，强度 **290%**、速度 **1.00×**；跨动画循环不重置相位，退场只被白场覆盖。详见[流光循环记录](docs/project-log/2026-09-22-flow-loop.md)。
...
1. 确认流光层次、真实手机可读性/性能、键盘和减弱动态体验。流光改为全程常开后，t=0 开场不再是纯白，X 会提前以灰影显形——这是“一直循环”的直接结果，需用户确认接受或另定形成阶段淡入。
...
构建通过；38个HTML内部链接与34个双语页面检查通过；Astro 48文件0错误/警告。浏览器实测 `/fr/`：连续采样14秒，流光强度恒为满值、流光时钟单调递增，并在 time 由22.87回绕到0.03的同一帧确认 flowTime 继续走到23.03，跨循环不重置；逐帧核对 t=0/11/20 的形成、完整X与退场白场；`/design-preview/` 空白浏览器读到23秒与290%默认值。尚未做实体手机GPU验收或远端部署验证。
