...
// The earlier quintic S curve was equivalent to power 1.875; 3 keeps that shape but rests
// longer at both ends and accelerates harder through the middle.
export const heroLightEase = { power: 3 } as const;
...
export function normalizeEase(value: {power?:number} = {}) {
  return { power:Number.isFinite(value.power)?Math.max(1,Math.min(8,value.power!)):heroLightEase.power };
}
...
export function motionEase(progress:number,power:number=heroLightEase.power){
  const t=Math.max(0,Math.min(1,progress));
  if(t<=0||t>=1)return t;
  const rise=Math.pow(t,power);
  return rise/(rise+Math.pow(1-t,power));
}
export function heroFrame(elapsed:number,timing:HeroTiming,reduced=false,power:number=heroLightEase.power){
  ...
  const assembly=time<exitStart?motionEase((time-timing.white)/timing.formation,power):1-motionEase((time-exitStart)/timing.exit,power);
  ...
  return {time,total,phase:reduced?'hold':phase,
    ...
    angle:reduced?0:Math.PI*(turns+motionEase(clamp((time-rotationStart)/timing.rotation),power))};
}
